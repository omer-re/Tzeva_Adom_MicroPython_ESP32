...
def start_thread_with_limit(target, args=()):
    ...
    def thread_limiter():
        ...
        target(*args)
        # Decrement the thread count after task completion
        active_threads -= 1

    if active_threads < MAX_THREADS:
        active_threads += 1  # Increment the active thread count
        _thread.start_new_thread(thread_limiter, ())
    else:
...
def log_and_print(*args):
    start_thread_with_limit(log_and_print_execution, args)
...
def print_text_processor():
    ...
    while run_print_processor:
        ...
        if print_queue:
            ...
            # If the queue is empty, just sleep for a bit
            sleep(0.1)
...
def print_text_rolling(name="■", row_height=0):
    ...
    if not rolling_text_lock.acquire(0):
        # print("exit 153")
        return
    ...
    try:
        ...
        for x in range(128, -len(name) * 8, -1):
            ...
            # if previous_x != 128:
            #    clear_area(previous_x, row_height, len(name) * 8, 8)
            display.text(name, x, row_height, 1)
            # if x % 4 == 0:  # Update the display less frequently
            display.show()
            previous_x = x
            sleep(0.001)
    finally:
...
def start_print_processor():
    ...
    with start_processor_lock:  # Acquire the lock
        ...
        start_thread_with_limit(print_text_processor, ())
...
class RedAlert():
    ...
    def process_alerts_string(self, parsed_str):
        ...
        if matching_pairs:
            ...
            start_thread_with_limit(alert_sound, ())
        ...
        try:
            if poll_for_alerts:
                ...
                try:
                    ...
                    start_thread_with_limit(print_text_rolling, ())
                    display_counter()
                    display_thread_counter()
                    # print(".", end='')
                    # print(ex)
